...
#browser()
  if("A" %in% models_to_run){
    Model_A <- brm(data = data_binomial, 
                   family = binomial,
                   res | trials(tot_samples) ~ 1 +
                     year_s + 
                     (1 | country) + 
                     (1 | laboratorycode),
                   file = paste0("fits/A_",pathogen_to_run,"_",resistance_to_run,"_", Sys.Date()), 
  ...
  if("B" %in% models_to_run){
   # browser()
    Model_B <- brm(data = data_binomial, 
                   family = binomial,
                   res | trials(tot_samples) ~ 1 +
                     age_s + 
                     age_squared_s + 
                     ...
                     (1 + age_s | country) + 
                     (1 | laboratorycode),
                   file = paste0("fits/B_",pathogen_to_run,"_",resistance_to_run,"_", Sys.Date()), 
                   file_refit = getOption("brms.file_refit", "always"),
                   iter = it_no, save_pars = save_pars(all = TRUE))
  }
  
  if("C" %in% models_to_run){
    Model_C <- brm(data = data_binomial, 
                   family = binomial,
                   res | trials(tot_samples) ~ 1 +
                     gender + 
                     year_s + 
                     (1  | country) + 
                     (1 | laboratorycode),
                   file = paste0("fits/C_",pathogen_to_run,"_",resistance_to_run,"_", Sys.Date()), 
                   file_refit = getOption("brms.file_refit", "always"),
                   iter = it_no, save_pars = save_pars(all = TRUE))
  }
  
  if(sensitivity_0 == "ON"){ 
   file_name_to_use <- "D_sensitivity0_" 
  ...
  if("D" %in% models_to_run){
    Model_D <- brm(data = data_binomial, 
                   ...
                   res | trials(tot_samples) ~ 1 +
                     ...
                     (1 + age_s | country) + 
                     (1 | laboratorycode),
                   file = paste0("fits/",file_name_to_use,pathogen_to_run,"_",resistance_to_run,"_", Sys.Date()),
                   file_refit = getOption("brms.file_refit", "always"),
                   iter = it_no, save_pars = save_pars(all = TRUE))
  }
} 